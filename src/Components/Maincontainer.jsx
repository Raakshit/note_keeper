...
import {AiOutlinePlus , AiOutlineSearch ,AiOutlineLeft,AiOutlineRight} from "react-icons/ai"
...
const Maincontainer = () => {
  ...
  return (
    <div className='flex flex-col bg-[#f4dec3] px-5 py-6 w-full h-screen'>
      ...
      <ToastContainer />
        ...
        <div className='flex items-center'>
          ...
          <AiOutlineSearch className=' absolute top-[123px] right-[443px]' size={19}/>
          <input 
          onChange={handleSearch}
          type="text" placeholder='Search title....' className='w-[270px] h-[35px] rounded-xl py-1 pl-8 bg-gray-200'/>
          </div>
          <button 
          onClick={toggleModal}
          className='flex items-center justify-evenly border-blue-600 border duration-500 bg-blue-600 text-white w-[150px] h-[35px] rounded-2xl hover:text-blue-700 hover:bg-white'><AiOutlinePlus/> Add Note</button>
        </div>
      </div>
      <div className=' my-2'>
        <div className='grid grid-cols-3'>
          ...
          onClick={toggleModal}
           className='flex items-center justify-evenly flex-col  bg-white m-6 w-[330px] h-[250px] rounded-lg cursor-pointer hover:text-blue-600 duration-300 hover:border hover:border-gray-400'>
            ...
            <div className=' hover:text-blue-600'>
              <p className='text-center text-xl my-2 font-bold'>Add New Note</p>
          ...
          <div className='fixed bottom-4 left-[650px]'>
            <ul className='flex items-center justify-center'>
              <li>
                <button className='flex items-center hover:bg-slate-100 rounded-[50%] p-2' onClick={prevPage}><AiOutlineLeft size={20}/></button>
              </li>
              {number?.map((n,i)=>(
                <li key={i}>
                  <button className={`h-[35px] w-[35px] bg-slate-100 rounded-[50%] mx-2 shadow-lg ${currentPage === n ? 'bg-blue-300' : ''}`} onClick={() => changeCpage(n)}>{n}</button>
                </li>
              ))}
              <li>
                <button className='flex items-center hover:bg-slate-100 rounded-[50%] p-2' onClick={nextPage}><AiOutlineRight size={20} /></button>
              </li>
            </ul>
          </div>
        </div>
